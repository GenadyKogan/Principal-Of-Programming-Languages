...
def calc_eval(exp):
    ...
    #############Solution 4 ##########################
    elif type(exp)==str:#if <exp> is a variable name
        if exp in env:
            return env[exp]
        else:
...
def calc_apply(operator, args):
    ...
    if operator in ('eq','=='):
        if len(args)!=2:
            raise TypeError(operator +'reguery 2 arguments')
        left,right=args
        return left==right
    if operator in ('gt','>'):
        if len(args)!=2:
...
def calc_parse(line):
    ...
    #############Solution 4  - distinguish between assognment and expression 
...
def tokenize(line):
    ...
    ['add', '(', '2', ',', 'mul', '(', '4', ',', '6', ')', ')']
    """
    #############Solution 4   add token '='
    spaced = line.replace('(',' ( ').replace(')',' ) ').replace(',', ' , ').replace('=',' = ')
    ######################################
    return spaced.strip().split()

known_operators = ['add', 'sub', 'mul', 'div', '+', '-', '*', '/','eq','gt','lt','>','<','==']
